...
# جلب البيانات من GitHub Secrets للأمان
...
def get_udemy_details(url):
    ...
    try:
        ...
        # محاولة استخراج رابط يودمي المباشر المتضمن للكوبون
...
def send_pro_message(course_data):
    ...
    caption = (
        ...
        f"📝 *Name:* {course_data['title_en']}\n\n"
        f"✅ *نبذة:* {course_data['summary_ar']}\n\n"
        f"⭐️ *التقييم:* {course_data['rating']} ({course_data['reviews']})\n"
        f"👥 *الطلاب:* {course_data['students']} | 🔥 شعبية عالية\n"
        f"⏳ *المدة:* {course_data['duration']}\n"
        f"💰 *السعر الأصلي:* {course_data['price']}\n"
        f"🏷 *التصنيف:* #{course_data['category']}\n\n"
        f"🔗 *رابط مباشر (مجاني 100%):*\n{course_data['link']}"
    )
    payload = {
        "chat_id": CHAT_ID,
    ...
    for source in RSS_SOURCES:
        ...
        for entry in feed.entries:
            if entry.link not in sent_courses:
                ...
                summary_ar = translate_text(entry.get('summary', 'كورس مميز على يودمي'))[:150] + "..."
